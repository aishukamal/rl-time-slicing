...
1. **The cross-pool sharing mechanics are proven at full pool utilization** — but this run measured *contention sharing*, not idle harvesting. Scale caveat, established from the solo run's own timings: at 0.5B on H100s, generation never starves the trainer (`timing_s/gen` ≈ 0.03 s per ~11.7 s step after warmup — trainer idle ≈ 0.3%, not the 30–74% seen at production scale). Both pools at ~99% therefore reflects two jobs time-sharing trainer capacity each wanted nearly full-time, with per-job throughput split accordingly. "Steps identical in duration to solo" is evidence of C/R and compute integrity, not of throughput-neutral sharing. The idle-recovery magnitude claim rests on production-scale trainer waits (SemiAnalysis: 30–74%), the regime where yield-on-starvation actually engages — demonstrating it empirically requires a workload with real generation-bound gaps (larger model / longer responses / agentic rollouts).
