...
# Changes: gym dependency dropped (plain class — nothing from gym.Env was
# used), logs/outputs root made overridable via DEEPRESEARCH_LOG_ROOT so the
# tree can live on a read-only mount.
...
class DeepResearchMultiProcessEnv:
    """
    ...
    def __init__(self,
        ...
        - group_n: Number of same environments in each group (for GRPO)
        - config: env configuration dictionary
        """
        ...
        sys.stderr.write(
            f"Init DeepResearchMultiProcessEnv, env_num: {env_num}, "
            f"group_n: {group_n}, num_processes: {env_num * group_n}\n")
    ...
    def step(self, original_responses, actions):
        ...
        :param actions: list, length must match self.num_processes
        ...
        print(f"Step {len(futures)} workers (env_num={self.env_num}, "
              f"group_n={self.group_n}, dataset_name={self.dataset_name})", file=sys.stderr)

        return obs_list, reward_list, done_list, info_list

    def reset(self, questions, question_ids, ground_truths=None, critiques=None):
        """
        Perform reset in parallel. Each question is assigned to group_n workers.
        """
        ...
        # Collect results
        print(f"Reset {len(futures)} workers (env_num={self.env_num}, "
              f"group_n={self.group_n}, dataset_name={self.dataset_name})", file=sys.stderr)
        results = ray.get(futures)
        obs_list = []
        info_list = []
...
def build_deepresearch_envs(
    ...
    log_root = os.environ.get("DEEPRESEARCH_LOG_ROOT", default_root)
    log_dir = os.path.join(log_root, f"deepresearch_logs/{dataset_name}/{time_str}")
    answer_dir = os.path.join(log_root, f"deepresearch_outputs/{dataset_name}/{time_str}")
