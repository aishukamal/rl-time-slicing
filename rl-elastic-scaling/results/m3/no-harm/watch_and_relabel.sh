...
POD=$($K --context=$CTX get pods -n default -l app=elastic-m3c -o jsonpath="{.items[0].metadata.name}")
OUT=/Users/aishuk/workspaces/GPU-CR/elastic-rl-poc/m3-results/no-harm/relabel.log
END=$(( $(date +%s) + 7200 ))
