"""reward.py — rule-based GSM8K reward and GRPO advantage computation
...
def extract_xml_answer(text: str) -> Optional[str]:
  ...
  if not m:
    return None
  raw = m.group(1).strip()
  # Extract the number from the answer (handles $42, 1,234, -5, 3.00)
...
def extract_answer(text: str) -> Optional[str]:
  ...
  Priority:
    ...
    1. <answer>N</answer>  (XML format — primary for this setup)
...
def is_truncated(text: str) -> bool:
  ...
  A completion that finished reasoning but forgot XML tags is NOT truncated —
  it's a format failure (reward=0.0) but not a length issue.
  """
  t = text.rstrip()
  ...
  if re.search(r'</answer>', t):
    return False
  # Completed with Qwen native formats
  ...
  if re.search(r'\*\*\s*\\?\$?\s*[-\d][,\d]*(?:\.\d+)?\s*\*\*', t):
    return False
  # Ends with a number — model concluded numerically
  if re.search(r'\d[.)!]?\s*$', t):
    return False
  # Ends with punctuation — model wrote a complete sentence, just wrong format
  if re.search(r'[.!?]\s*$', t):
    return False
  # Has </reasoning> — model finished reasoning, just forgot <answer>
  # This is a format failure, not truncation
  if re.search(r'</reasoning>', t):
    return False
  # Anything else ending mid-sentence is genuinely truncated
  return True
...
def has_xml_format(text: str) -> bool:
  ...
  Since <reasoning> is pre-filled in the prompt, we only check that the
  completion contains </reasoning> and <answer>N</answer>.
  """
  return bool(
...
def compute_rewards(
  ...
  """Reward function matching willccbb's validated setup:
    ...
    2.0 = correct answer (with or without format)
    0.5 = correct XML format, wrong answer
    0.0 = wrong answer, no format

  Stats:
    correct_rate   — fraction with correct answer
    format_rate    — fraction with correct XML structure
    truncated_rate — fraction cut off (diagnostic)
  """
  rewards = []
  ...
  for comp, gt in zip(completions, ground_truths):
    ...
    if correct:
      reward = 2.0  # correctness reward
    elif has_fmt:
      reward = 0.5  # format reward — model learned structure, not yet math
    else:
      reward = 0.0
