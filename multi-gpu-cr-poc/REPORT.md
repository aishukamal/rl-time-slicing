...
- **NVIDIA `contrib/nccl_checkpoint`** (NCCL ≥ 2.30.7): **API-driven** (`ncclCheckpointPrepare()`/`ncclCheckpointRestore()` called in-process). Richer replay engine — CommSplit/shrink/grow, `ncclCommRegister` buffers, window registration, synthetic-handle indirection from creation, multi-host Redis rendezvous (survives IP changes). Gaps: no external trigger (needs a signal→API companion for our snapshot-agent), no ctypes/dlopen coverage (**tested, not just inferred**: cannot serve as `VLLM_NCCL_SO_PATH` — three independent blockers, each sufficient: 5 PyNCCL-required symbols missing from its export list (`ncclGetVersion/GetUniqueId/GetErrorString/GroupStart/GroupEnd` — their generator only exports handle-taking functions), bare dlopen fails under RTLD_NOW (unresolved refs to real NCCL), and RTLD_NEXT forwarding fails from a dlopen'd position even with libnccl preloaded; evidence in `nccl-checkpoint-tests/envvar-test/RESULTS.md`. **Warning: the failure is silent** — vLLM 0.27 swallows the pynccl load error, falls back to torch.distributed with "Using [] all-reduce backends", and everything appears to work on a degraded TP path), no CUDA-graph handling, and a config version-skew bug we patched (hits any torch built against older NCCL headers — patch at `nccl-checkpoint-tests/nvidia-shim-eval/shim-version-skew-patch.diff`). **Verdict: adopt with wrappers** — see "NVIDIA contrib/nccl_checkpoint Shim Evaluation" below.
...
- **FSDP DP=2: PASS** (2 cycles, shim tracks and replays the comms — its replay engine doing real work). **vLLM TP=2: PASS** (2 cycles) — but requires worker-side plumbing: vLLM's PyNCCL loads NCCL via ctypes (bypasses LD_PRELOAD), so a `--worker-extension-cls` extension must destroy/recreate the pynccl comm around Prepare/Restore (reusable extension: `nccl-checkpoint-tests/nvidia-shim-eval/ckpt_ext.py`). **Caveat discovered in follow-up testing (envvar-test, 2026-09-09): in this vLLM config the NVIDIA shim tracks ZERO comms** — Prepare/Restore short-circuit as no-ops (`g_CommCheckpointCount==0`, empty Redis) and the entire pass is carried by the extension's manual pynccl destroy/recreate. So for vLLM, their shim currently contributes nothing; its replay engine is only exercised by torch.distributed-style comms (FSDP path).
