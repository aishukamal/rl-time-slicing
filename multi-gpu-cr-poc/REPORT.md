...
6. **`--disable-custom-all-reduce` required on driver 580 (Option B).** Root cause proven with minimal reproducer: `cudaIpcOpenMemHandle` creates cross-process GPU memory mappings that driver 580's cuda-checkpoint cannot restore (thaw returns "invalid argument"). Both vLLM and SGLang's custom all-reduce calls `cudaIpcOpenMemHandle` to map peer GPU memory into each rank's address space. Closing IPC handles before freeze (`cudaIpcCloseMemHandle`) fixes the thaw — but frameworks load these via ctypes, bypassing LD_PRELOAD interception. Driver 580's cuda-checkpoint README explicitly states it "does not support UVM memory or IPC memory." **Driver 610 adds native `cuIpcGetMemHandle`-based IPC checkpoint/restore support** ([cuda-checkpoint README](https://github.com/NVIDIA/cuda-checkpoint), [issue #56](https://github.com/NVIDIA/cuda-checkpoint/issues/56)) — testing in progress. Performance impact of `--disable-custom-all-reduce`: negligible (<1-2%, only affects small all-reduces).
