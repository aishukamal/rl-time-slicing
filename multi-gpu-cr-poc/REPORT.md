...
6. **`--disable-custom-all-reduce` required for cuda-checkpoint (Option B).** Both vLLM and SGLang use a custom all-reduce implementation that creates cross-process GPU state loaded via ctypes (bypassing LD_PRELOAD). With custom all-reduce enabled, cuda-checkpoint thaw fails; with it disabled, thaw succeeds. The exact GPU state that breaks restore has not been isolated with a minimal reproducer. Performance impact of disabling: negligible (<1-2% throughput, only affects small all-reduces where NCCL launch overhead dominates).
